...
export function TimelineTrack({ sessions, onSessionClick, onTrackClick, onSessionMove }: TimelineTrackProps) {
  const trackRef = useRef<HTMLDivElement>(null);
  const ghostRef = useRef<HTMLDivElement>(null);

  const { onMouseDown } = useDragToSelect(trackRef, ghostRef, onTrackClick);
